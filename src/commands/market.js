...
module.exports = {
  ...
  dbl: false, 
  name: ['market'],  
  description: '2313 Uptime marketini görüntülersiniz.',
...
async execute(client, interaction) { 
  ...
  const Butonlar = new ActionRowBuilder()
    ...
    new ButtonBuilder()        
      .setURL(`https://discord.gg/XrpG6U7tPa`)
    ...
    .addFields(
      {
        ...
        value: `+1 link ekleme hakkı - **30 AC**`
      },
      {
        name: `${Emojis.Elmas} Premium üyelik`,
        value: `1 aylık premium üyeliği - **300 AC**`
      },
      {
        name: `<a:premium:1182687394173038682> Armoon Coin(AC)`,
        value: `AC almak için destek sunucusunda kurucu veya geliştiriciler ile iletişime geçiniz.`
      })
    ...
    if(interaction.customId === `linklimital_${interaction.user.id}`) {
        ...
        await interaction.awaitModalSubmit({ filter: (interaction) => interaction.customId === `limitalmaform`, time: 60000 }).then(async (interaction) => {
          ...
          const ParaYok = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Almak istediğin **${Limit}** link limit hakkı için **${Limit*30} AC** gerekli.`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
          ...
          const Alındı = new EmbedBuilder()
            ...
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Tik} **${Limit}** link limit hakkı **${Limit*30} AC** karşılığında satın alındı.`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
      ...
      if(interaction.customId === `premiumal_${interaction.user.id}`) {
        ...
        if(Premium) {
          ...
          const ParaYok = new EmbedBuilder()
            .setColor(Colors.Red)
            .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
            .setDescription(`${Emojis.Çarpı} Premium almak için **300 AC** gerekli.`)
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
