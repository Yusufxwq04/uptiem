...
module.exports = {
  ...
  dbl: false, 
  name: ['sıralama'],  
  description: 'AC zenginlerinin sıralamasını gösterir.',
...
async execute(client, interaction) {
  ...
  let filtre = client.users.cache.filter(x => (db.fetch(`Bakiye_${x.id}`)) || 0)
    ...
    .map((x) => {
      return `**${sayı++}.** ${x.username} - ${db.fetch(`Bakiye_${x.id}`) || 0} AC`
    })
  
  const Sıralama = new EmbedBuilder()
    .setColor(Colors.Yellow)
    .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
    .setDescription(`> Armoon Coin (AC) zenginlerinin sıralaması`)
    .addFields(
      {
