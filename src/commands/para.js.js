...
async execute(client, interaction) { 
  ...
  const Sahip = new EmbedBuilder()
    ...
    .setTimestamp()
  if(interaction.user.id !== "824626594224013322" && interaction.user.id !== "824626594224013322" && interaction.user.id !== "824626594224013322") return await interaction.followUp({embeds: [Sahip]})
  ...
  if(interaction.options.getSubcommand() === 'ekle') {
    ...
    const Eklendi = new EmbedBuilder()
      ...
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Tik} ${Kullanıcı} adlı kullanıcının bakiyesine **${Miktar} AC** eklendi.`)
       .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
  ...
  if(interaction.options.getSubcommand() === 'al') {
    ...
    const ParaYok = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Bu kullanıcının bakiyesinde **${Miktar} AC** bulunmuyor.`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const Alındı = new EmbedBuilder()
      .setColor(Colors.Green)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Tik} ${Kullanıcı} adlı kullanıcının bakiyesinden **${Miktar} AC** alındı.`)
       .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
